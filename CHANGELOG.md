...
- Repository-local native Dependabot auto-merge and inbound-rights documentation, following the approved Enterprise/Organization reform standard.
- PR validation through `CI`, `Build Pages artifact`, `Dependency Review` and `Run zizmor`; the rollout requires their native GitHub required-check configuration before merge.
...
- Complete browser-bundle license notices generated by Vite's official `build.license` and published through the legal surface. The Apache-2.0 election for DOMPurify is retained; custom inventory/artifact validation is retired by this reform.

### Changed

- Linear Release uses the official action and CLI v0.17.2, preserving exact deployed-SHA checkout, least-privilege permissions and its dedicated environment. Only successful push-triggered Deploy runs on this repository's `main` create a release; manual Deploy runs do not. Pending runs use `queue: max`, and action failures remain visible.
- Security, dependency review and Cloudflare deployment workflows use official upstream actions directly. CodeQL uses Default Setup; Scorecard uploads local SARIF without public-results publishing. The Pages D1 binding remains in Cloudflare's dashboard; nonsecret identifiers are permitted by the current policy, but no binding migration is made.
- Dependabot uses a weekly Monday schedule at 06:00 America/Sao_Paulo, seven-day cooldown (excluding `actions/*` and `github/*`), automatic rebases and minor/patch groups; major version updates remain separate.
- Cloudflare publication retains Pages Functions, existing credentials and the `calculadora-app` project, selects `main` explicitly and pins Wrangler 4.129.1, the version observed in the last successful deployment. No standalone Worker, D1 migration or npm publication flow is introduced.
...
- The custom Auto-release and legacy auto-merge workflows, merge-queue triggers, disabled advanced CodeQL workflow and separate Public Format gate. Native GitHub auto-merge replaces the retired merge queue.
- Custom third-party inventory/artifact verifier, its dedicated tests and package-script consumers. Native Vite notice generation and the application legal module remain; automated parity and artifact checks are no longer part of CI. Product D1 and version-consistency tests are retained.
