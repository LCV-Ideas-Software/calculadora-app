...
`;
    ...
    try {
      ...
      if (inputTokens > GEMINI_CONFIG.maxTokensInput) {
        return jsonResponse(
          { erro: `Input exceeds token limit: ${inputTokens} > ${GEMINI_CONFIG.maxTokensInput}` },
          413,
        );
      }
    } catch (countError) {
    ...
    const payloadCandidates: PayloadCandidate[] = [
      ...
      },
      {
        label: 'compat',
        systemInstruction:
          'Você é um analista financeiro sênior especializado em operações de câmbio para pessoas físicas no Brasil.',
        config: {
      ...
      for (let tentativa = 0; tentativa < GEMINI_CONFIG.maxRetries; tentativa++) {
        try {
          const response = await ai.models.generateContent({
            model: modelName,
            contents:
              candidate.label === 'minimal'
                ? `Contexto de atuação: Você é analista financeiro...\n\n${promptText}`
                : promptText,
            config: {
              ...candidate.config,
          ...
          if (err instanceof Error) {
            structuredLog('warn', 'Oráculo Gemini request error', {
              endpoint: 'oraculo',
              error: err.message,
              payload: candidate.label,
              attempt: tentativa + 1,
